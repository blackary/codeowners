...
# /// script
# requires-python = ">=3.9"
# dependencies = ["codeowners-cli"]
# ///
"""
Look up CODEOWNERS for one or more files.

Usage:
    codeowners.py [--codeowners PATH] [--only-owners] [--output FORMAT] [--no-header] FILE [FILE ...]

Output formats: text (default), csv, tsv, json
"""
...
def main() -> None:
    ...
    parser.add_argument("--codeowners", metavar="PATH")
    parser.add_argument("--only-owners", action="store_true")
    parser.add_argument(
        "--output", choices=["text", "csv", "tsv", "json"], default="text"
    )
    ...
    for filename in args.filenames:
        owner_names = [o[1] for o in owners.of(filename)]
        if args.only_owners and not owner_names:
            continue
        rows.append((filename, owner_names))

    if args.output == "json":
        print(json.dumps([{"filename": f, "owners": o} for f, o in rows], indent=2))
